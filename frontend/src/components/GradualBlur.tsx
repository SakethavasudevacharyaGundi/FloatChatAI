...
const CURVE_FUNCTIONS: any = {
  linear: (p: any) => p,
  bezier: (p: any) => p * p * (3 - 2 * p),
  'ease-in': (p: any) => p * p,
  'ease-out': (p: any) => 1 - Math.pow(1 - p, 2),
  'ease-in-out': (p: any) => (p < 0.5 ? 2 * p * p : 1 - Math.pow(-2 * p + 2, 2) / 2)
};

const mergeConfigs = (...configs: any) => configs.reduce((acc: any, c: any) => ({ ...acc, ...c }), {});
