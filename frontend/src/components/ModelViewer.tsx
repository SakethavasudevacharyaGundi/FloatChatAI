...
'use client';
import { Suspense, useRef, useLayoutEffect, useEffect, useMemo, ReactNode } from 'react';
  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <div style={{ color: 'white', fontFamily: 'sans-serif', padding: '10px 20px', background: 'rgba(0,0,0,0.5)', borderRadius: '20px' }}>
          {Math.round(progress)} %
        </div>
      )}
    </Html>
...
const DesktopControls = ({ pivot, min, max, zoomEnabled }: any) => {
...
const ModelInner = ({
  ...
  useLayoutEffect(() => {
    ...
    // Slight delay to allow children to mount before measuring bounding sphere
    setTimeout(() => {
      ...
      g.traverse(o => {
        if ((o as THREE.Mesh).isMesh) {
          ...
          if (fadeIn && (o as THREE.Mesh).material) {
        ...
        const id = setInterval(() => {
          ...
          g.traverse(o => {
            if ((o as THREE.Mesh).isMesh && (o as THREE.Mesh).material) {
              ((o as THREE.Mesh).material as THREE.Material).opacity = v;
            }
          });
          invalidate();
          if (v === 1) {
