...
class ChartGenerator:
    ...
    def _get_depth_range(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        return {}
    def generate_streamlit_visualization(self,visualization_analysis, data):
        try:
            result=asyncio.run(
                self.generate_chart(
            ...
            return result.get("chart_object", None)
        except Exception as e:
            logger.error(f"Streamlit visualization generation failed: {str(e)}")
            return None
